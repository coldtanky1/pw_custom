...
class Military(commands.Cog):
    ...
    async def recruit(self, ctx, amount: int):
        ...
        if result:
            ...
            cursor.execute(
                'SELECT name, troops, planes, weapon, tanks, artillery, anti_air, barracks FROM user_mil WHERE name = ?',
                (name,))
    ...
    async def allocate(self, ctx, mil_type: str, amount: int):
        ...
        if result:
            ...
            if mil_result and res_result and infra_result:
                ...
                name, wood, coal, iron, lead, bauxite, oil, uranium, food, steel, aluminium, gasoline, ammo, concrete = res_result
                infra_name, basic_house, small_flat, apt_complex, skyscraper, lumber_mill, coal_mine, iron_mine, lead_mine, bauxite_mine, oil_derrick, uranium_mine, farm, aluminium_factory, steel_factory, oil_refinery, ammo_factory, concrete_factory, militaryfactory, corps = infra_result
